...
import { facebookRegisterAsync, fakelookRegisterAsync, googleRegisterAsync } from '../../../services/Authentication/register.service';
...
const Register = props => {
    ...
    const registerWithFakelook = (email, password, confirmPassword, user, address) => {
        fakelookRegisterAsync(email, password, confirmPassword, user, address)
    ...
    const registerWithFacebook = (response, user, address) => {
        ...
        facebookRegisterAsync(accessToken, facebookId, user, address)
            .then(res => {
                history.push('/login');
            })
            .catch(err => {
    ...
    const registerWithGoogle = (response, user, address) => {
        ...
        googleRegisterAsync(tokenId, user, address)
            .then(res => history.push('/login'))
            .catch(err => setError(err.message));
    }
