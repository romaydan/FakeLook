...
const dbTest = async () => {
  const post = await Post.create({
    ...
    publisherId: '123',
    location: { type: 'Point', coordinates: [34.87711793780242, 32.02717259144145] },
  });
    ...
    include: [
      {
        ...
        through: {
          attributes: [],
          where: []?.length > 0 ? { tagId: { [Op.in]: [] } } : { tagId: { [Op.not]: null } },
        },
      },
      {
        model: UserTag,
    ...
    where: {
      [Op.and]: [
        sequelize.fn(
          ...
          sequelize.fn('ST_SetSRID', sequelize.fn('ST_MakePoint', 34.8754289039092, 32.02753787187709), 4326),
          1000,
          false
        ),
        {
